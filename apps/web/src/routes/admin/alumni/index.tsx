...
function AlumniPage() {
	...
	const toggleMutation = useMutation({
		mutationFn: (input: {
			...
			value: boolean;
		}) => client.admin.alumni.toggle(input),
		onSuccess: () => invalidate(),
		onError: (e: Error) => toast.error(e.message),
	});

	const deleteMutation = useMutation({
		mutationFn: (id: string) => client.admin.alumni.delete({ id }),
		onSuccess: () => {
	...
	const bulkMutation = useMutation({
		...
		}) => client.admin.alumni.bulkAction(input),
		onSuccess: (_, vars) => {
			toast.success(
				vars.action === "delete"
					? `${vars.ids.length} alumni dihapus`
					: `${vars.ids.length} alumni di-${vars.action}`,
			);
			setBulkDeleteIds([]);
			invalidate();
		},
	...
	const columns: ColumnDef<AlumniRow>[] = [
		{
			id: "alumni",
			header: "Alumni",
			cell: ({ row }) => (
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-xs text-muted-foreground">{row.original.batchLabel}</p>
					</div>
				</div>
			),
		},
		...
		},
		{
			...
			cell: ({ row }) => (
				<span className="text-sm text-muted-foreground">
					{new Date(row.original.graduatedAt).toLocaleDateString("id-ID", {
						month: "short",
						year: "numeric",
					})}
				</span>
	...
	const items =
		(query.data as { items: AlumniRow[]; total: number } | undefined)?.items ?? [];
	const total =
		(query.data as { items: AlumniRow[]; total: number } | undefined)?.total ?? 0;
	...
	return (
		<>
			<AdminDataTable
				...
				filters={[
					{ key: "programSlug", label: "Program", options: PROGRAM_OPTIONS },
					{
						key: "status",
						label: "Status",
						options: [
				...
				onBulkAction={(action, ids) => {
					if (action === "delete") setBulkDeleteIds(ids);
					else bulkMutation.mutate({ ids, action: action as "publish" | "unpublish" });
				}}
				onAdd={() =>
					navigate({ to: "/admin/alumni/$id", params: { id: "new" } })
				}
				...
				emptyTitle="Belum ada alumni"
				emptyDescription="Tambahkan profil alumni pertama Momkiddis."
			/>
			...
			<AdminConfirmDialog
				...
				title={`Hapus ${bulkDeleteIds.length} Alumni?`}
				description="Semua alumni yang dipilih akan dihapus permanen."
				confirmLabel="Ya, Hapus Semua"
				variant="destructive"
				isLoading={bulkMutation.isPending}
				onConfirm={() => bulkMutation.mutate({ ids: bulkDeleteIds, action: "delete" })}
			/>
		</>
	);
}
